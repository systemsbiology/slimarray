...
describe ChipReturnsController do
  ...
  before(:each) do
    ...
    @mock_chip_return = mock("Chip purchase", :from_lab_group_id => 42, :chip_type_id => 23)
  end

  describe "responding to GET new" do

    before(:each) do
      ChipReturn.should_receive(:new).
        with(:from_lab_group_id => "42",:chip_type_id => "23").
        and_return(@mock_chip_return)
    end
  ...
  describe "responding to POST create" do
    ...
    before(:each) do
      ChipReturn.should_receive(:new).with("params").and_return(@mock_chip_return)
    end

    def do_post
      post :create, :lab_group_id => 42, :chip_type_id => 23, :chip_return => "params"
    end

    context "with a successful save" do

      before(:each) do
        @mock_chip_return.should_receive(:valid?).and_return(true)
        @mock_chip_return.should_receive(:save).and_return(true)
    ...
    context "with a failed save" do
      ...
      before(:each) do
        @mock_chip_return.should_receive(:valid?).and_return(false)
      end
